...
namespace SimpleFootBallModel
{
	...
	class OddsGenerator
	{
		...
		private void printResult(double[] homeWin, double[] awayWin, double[] draw, double[] hilo) {
			...
			Console.WriteLine("| " + hiloTarget + "  | " +oddsFormat(probabilityToOdds(hilo[OVER_TARGET], margin)) + " | " + oddsFormat(probabilityToOdds(hilo[UNDER_TARGET], margin)) + " |");
			Console.WriteLine();

			Console.WriteLine("| Line | High | Low  | HiLo (WithOut Margin)");
			Console.WriteLine("| " + hiloTarget + "  | " +oddsFormat(probabilityToOdds(hilo[OVER_TARGET], 0.0)) + " | " + oddsFormat(probabilityToOdds(hilo[UNDER_TARGET], 0.0)) + " |");
			Console.WriteLine();
		}
		...
		public void generateOdds()
		{
			...
			double[] hilo = generateHiLoProbability(hiloTarget);
